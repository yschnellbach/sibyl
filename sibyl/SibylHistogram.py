...
class SibylHistogram(gl.GLViewWidget):
    ...
    def _axes(self):
        ...
        self.ax.translate(5.5 / 2, 10 / 2.0, 0)
    ...
    def drawLegend(self):
        ...
        pixRatio = super(
            SibylHistogram, self
        ).devicePixelRatio()  # needed to scale font size and wstart
        ...
        self.renderText(
            wstart / pixRatio, 0.1 * self.height(fixed=True), cmask, font=font
        )
        self.renderText(
            wstart / pixRatio, 0.2 * self.height(fixed=True), self.txt, font=font
        )
